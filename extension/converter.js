...
(function (root) {
  ...
  async function extractText(file) {
    ...
    if (name.endsWith('.pdf')) return await pdfText(await file.arrayBuffer());
  ...
  async function convert(file, mode) {
    ...
    if (!text.trim()) throw Error('No se encontró texto seleccionable. Los PDF escaneados necesitan OCR.');
  ...
  async function analyzeDocument(file) {
    const text = await extractText(file);
    if (!text.trim()) throw Error('No se encontró texto seleccionable. Para PDF escaneados haría falta OCR.');
